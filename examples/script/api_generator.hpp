...
public:
	...
	static std::string generate_cpp_api() {
		...
		// Add user-defined header content
		output += HostBindings::get_header();
		output += "\n";
...
private:
	...
	static std::string translate_cpp_to_rust_signature(const std::string& cpp_sig) {
		...
		std::string return_type_rust = translate_cpp_type_to_rust(return_type_cpp);

		// Translate arguments
		std::string args_rust = translate_cpp_args_to_rust(args_cpp);
	...
	static std::string translate_cpp_type_to_rust(const std::string& cpp_type) {
		...
		if (type.find("std::vector") != std::string::npos) {
			return "*const core::ffi::c_void"; // Generic pointer for now
		}
	...
	static std::string translate_cpp_args_to_rust(const std::string& cpp_args) {
		if (cpp_args.empty()) {
			return "";
		}
		...
		for (size_t i = 0; i < args.size(); ++i) {
			...
			// Translate type
			std::string rust_type = translate_cpp_type_to_rust(arg_type);
